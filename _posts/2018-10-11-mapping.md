...
* We use **QGIS** and **PostGIS** in the data investigations class because it is much more powerful analytical tool. The learning curve is a bit steep, however, and QGIS doesn't produce  interactive maps. **R** is powerful and not easy to learn, but also not impossible to learn. And you can use **R** to do geographical analysis. R and QGIS will both generate SVG files that you can style in Illustrator or any other vector graphics editor for publication (you can also control style some within those two tools but if you're making a production graphic you definitely will have an easier time if you just use an illustration tool to fine tune the image). 
